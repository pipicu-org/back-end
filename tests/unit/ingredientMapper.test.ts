...
describe('IngredientMapper', () => {
  ...
  describe('requestDTOToEntity', () => {
    it('should create ingredient entity from request DTO', () => {
      const requestDTO: IngredientRequestDTO = new IngredientRequestDTO(
        'Tomato',
        2.5,
        1,
        20,
      ...
      expect(result.stock).toBe(requestDTO.stock);
      expect(result.cost).toBe(requestDTO.cost);
    });
  });
  ...
  describe('toEntity', () => {
    it('should create ingredient entity from response DTO', () => {
      ...
      expect(result.unitId).toBe(responseDTO.unit.id);
      expect(result.lossFactor).toBe(responseDTO.lossFactor);
    });
  });
  ...
  describe('toResponseDTO', () => {
    it('should map ingredient entity to response DTO', () => {
      ...
      expect(result.unit.id).toBe(ingredient.unit.id);
  ...
  describe('createSearchToIngredientSearchDTO', () => {
    it('should create search response DTO', () => {
      ...
      const result = ingredientMapper.createSearchToIngredientSearchDTO(
        [ingredients, total],
        search,
        page,
        limit,
      ...
      expect(result.data[0].unit.id).toBe(ingredients[0].unit.id);
